...
# Instructions:
#
# 1. Change the value of the cls_urn variable on line 172 to match the CLS you are wanting to export emails for
# 2. Save your changes
# 3. Open Terminal
...
# 
# Note: Any new executions of this script will overwrite the specified client's data csv. 
# 
#
	...
	hub_data["client"]["locations"].each do |loc|
		# grab locations that are not deleted or suspended
			...
			cls_data["configurable_attributes"].each do |item|
				...
				if item["category"] == "Location" && item["location_urn"] == email_obj.get_loc_urn
